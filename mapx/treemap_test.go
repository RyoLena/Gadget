...
func TestNewTreeMapWithMap(t *testing.T) {
	tests := []struct {
		name       string
		m          map[int]int
		comparable Gadget.Comparator[int]
		wantKey    []int
		wantVal    []int
		wantErr    error
	}{
		{
			...
			wantKey:    nil,
			wantVal:    nil,
			wantErr:    errors.New("Gadget: Comparator不能为nil"),
		},
		{
			name:       "empty",
			m:          map[int]int{},
...
// goos: windows
// goarch: amd64
// pkg: github.com/gotomicro/Gadget/mapx
...
// PASS
// ok      github.com/gotomicro/Gadget/mapx  0.797s
func BenchmarkTreeMap(b *testing.B) {
	//hashmap := NewHashMap[hashInt, int](10)
	treeMap, _ := NewTreeMap[uint64, int](Gadget.ComparatorRealNumber[uint64])
