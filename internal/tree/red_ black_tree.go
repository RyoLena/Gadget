...
var (
	ErrRBTreeSameRBNode = errors.New("gadget: RBTree不能添加重复节点Key")
	ErrRBTreeNotRBNode  = errors.New("gadget: RBTree不存在节点Key")
	// errRBTreeCantRepaceNil = errors.New("gadget: RBTree不能将节点替换为nil")
)

type RBTree[K any, V any] struct {
	root    *rbNode[K, V]
	compare Gadget.Comparator[K]
	size    int
}
...
// NewRBTree 构建红黑树
func NewRBTree[K any, V any](compare Gadget.Comparator[K]) *RBTree[K, V] {
	return &RBTree[K, V]{
		compare: compare,
		root:    nil,
	}
