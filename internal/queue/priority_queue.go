...
var (
	ErrOutOfCapacity = errors.New("gadget: 超出最大容量限制")
	ErrEmptyQueue    = errors.New("gadget: 队列为空")
)
...
func NewPriorityQueue[T any](capacity int, compare Gadget.Comparator[T]) *PriorityQueue[T] {
	sliceCap := capacity + 1
	if capacity < 1 {
		capacity = 0
		sliceCap = 64
