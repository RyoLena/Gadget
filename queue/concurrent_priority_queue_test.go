...
var (
	errOutOfCapacity = ErrOutOfCapacity
...
func TestNewConcurrentPriorityQueue(t *testing.T) {
	testCases := []struct {
		...
		{
			name:     "无边界",
			q:        NewConcurrentPriorityQueue(0, Gadget.ComparatorRealNumber[int]),
			capacity: 0,
		...
		{
			name:     "有边界 ",
			q:        NewConcurrentPriorityQueue(6, Gadget.ComparatorRealNumber[int]),
			capacity: 6,
			data:     []int{6, 5, 4, 3, 2, 1},
			expect:   []int{1, 2, 3, 4, 5, 6},
		},
...
func TestConcurrentPriorityQueue_Enqueue(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			q := NewConcurrentPriorityQueue[int](tc.capacity, Gadget.ComparatorRealNumber[int])
			wg := sync.WaitGroup{}
...
func TestConcurrentPriorityQueue_Dequeue(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			q := NewConcurrentPriorityQueue[int](tc.total, Gadget.ComparatorRealNumber[int])
...
func TestConcurrentPriorityQueue_EnqueueDequeue(t *testing.T) {
	...
	for _, tt := range testCases {
		tc := tt
		t.Run(tc.name, func(t *testing.T) {
			q := NewConcurrentPriorityQueue[int](0, Gadget.ComparatorRealNumber[int])
...
func ExampleNewConcurrentPriorityQueue() {
	q := NewConcurrentPriorityQueue[int](10, Gadget.ComparatorRealNumber[int])
	_ = q.Enqueue(3)
	_ = q.Enqueue(2)
	_ = q.Enqueue(1)
	var vals []int
